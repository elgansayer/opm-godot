...
void Configurator::WriteData(char **data, size_t *size)
{
    ...
    for (i = 1; i <= m_reverseSections.NumObjects(); i++) {
        ...
        if (!FindData(LINE_SECTION, section->name, NULL, &offset, currentData, currentSize)) {
            ...
            Com_sprintf(
                currentData + offset, currentSize + 1 - offset, offset ? "\n\n[%s]" : "[%s]", section->name.c_str()
            );
        ...
        for (j = 0; j < section->key.NumObjects(); j++) {
            ...
            if (key->m_bArray) {
                for (k = 0; k < key->value.NumObjects(); k++) {
                    ...
                    bFound = FindData(
                        LINE_VALUE, section->name, key->name + "[" + k + "]", &offset, currentData, currentSize
                    );
                    {
                        int digits, l;
                        ...
                        len = 1 + key->name.length() + 1 + digits + 1 + 1
                            + value.length(); // \n + name + [ + digits + ] + = + value
                        currentSize += len;
                        currentData = new char[currentSize + 1];
                        strncpy(currentData, oldData, offset);
            ...
            if (key->m_bArray) {
                for (k = 0; k < key->value.NumObjects(); k++) {
...

                    {
                        ...
                        tlen += key->name.length() + 1 + digits + 1 + 1 + value.length()
                              + 1; // name + [ + digits + ] + = + value + \n
                        if (tlen > currentSize) {
                            oldData = currentData;
                        ...
                        p += Com_sprintf(
                            p, currentSize - (p - currentData), "%s[%d]=%s\n", key->name.c_str(), k, value.c_str()
                        );
                    }
                }
            } else {
