/* OPM_BOOT_START */
		...
		(async function opmBoot() {
			...
			var gameSel = document.getElementById('opm-target-game');
			var loader = document.getElementById('opm-loader');
			var pickSection = document.getElementById('opm-pick-section');
			var playBtn = document.getElementById('opm-play-btn');
			var cacheStatus = document.getElementById('opm-cache-status');
			if (gameSel) gameSel.value = String(targetGame);

			function dbName(gd) { return 'opm-' + gd; }
			...
			async function opmReadDirHandle(h, prefix) {
				var files = {};
				for await (var entry of h.values()) {
					if (entry.name.startsWith('.')) continue;
					var p = prefix ? (prefix + '/' + entry.name) : entry.name;
					if (entry.kind === 'directory') {
						Object.assign(files, await opmReadDirHandle(entry, p));
					} else {
						try {
			...
			async function opmReadViaInput() {
				return new Promise(function(ok) {
					var inp = document.getElementById('opm-input-fb');
					inp.onchange = async function() {
						var list = Array.from(inp.files);
						var files = {};
						var area = document.getElementById('opm-prog-area');
						var bar = document.getElementById('opm-file-prog');
						var txt = document.getElementById('opm-prog-text');
			...
			async function opmPickFiles() {
				...
				if (window.showDirectoryPicker) {
					try {
						...
						var files = await opmReadDirHandle(dh, '');
						area.style.display = 'none';
				...
				return opmReadViaInput();
			}

			function opmMapFilesToMemfs(rawFiles) {
				var mapped = {};
				var keys = Object.keys(rawFiles);
				var foundDirs = {};
				for (var i = 0; i < keys.length; i++) {
			...
			function startEngine() {
				...
				window.__opmTargetGame = targetGame;
				window.__opmGameDir = GAME_DIRS[targetGame] || 'main';
			...
			await new Promise(function(resolve) {
				playBtn.onclick = async function() {
					...
					document.getElementById('opm-pick-btn').onclick = async function() {
						this.disabled = true;
						var rawFiles = await opmPickFiles();
						if (rawFiles) {
							targetGame = parseInt(gameSel.value, 10) || 0;
							window.__opmTargetGame = targetGame;
							window.__opmGameDir = GAME_DIRS[targetGame] || 'main';
							var files = opmMapFilesToMemfs(rawFiles);
							var n = Object.keys(files).length;
							console.log('[MOHAAjs] Read ' + n + ' local files (target: ' + GAME_NAMES[targetGame] + ')');
							window.__opmLocalFiles = files;
							loader.style.display = 'none';
				...
				document.getElementById('opm-skip-btn').onclick = function() {
					targetGame = parseInt(gameSel.value, 10) || 0;
					window.__opmTargetGame = targetGame;
					loader.style.display = 'none';
					resolve();
